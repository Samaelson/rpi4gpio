//#define LOG_NDEBUG 0
#define LOG_TAG "RPI4GPIO_DEMOSERVICE"  
...
void* hidlThread_work(void* param) {
	...
	sp<IRpi4gpio> service = IRpi4gpio::getService();
	if( service == nullptr ){
		ALOGE("Can't find IRpi4gpio service...");
		return NULL;
	}
...
DemoService::DemoService()
{
		mHaveNotify = false;
		mSwStatus = -1;
        mGpioCallback = new ::android::hardware::rpi4gpio::V2_0::implementation::Rpi4gpioCallback();
        sp<::android::hardware::rpi4gpio::V2_0::IRpi4gpio> service = IRpi4gpio::getService();
		if( service == nullptr ){
			ALOGE("Can't find IRpi4gpio@1.1 service...");
		}
		else{
...
::android::binder::Status DemoService::SetLed(int32_t a, int32_t* _aidl_return)
{
	...
	if( service == nullptr ){
		ALOGE("Can't find IRpi4gpio@2.0 service...");
		//return -1;
